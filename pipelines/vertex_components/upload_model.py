...
def upload_model(
    ...
    def lookup_model(project_id: str, project_location: str, model_name: str) -> aip.Model:
        ...
        if len(models) == 0:
            logging.info(
                f"No model found with name {model_name}" + f"(project: {project_id} location: {project_location})"
            )
            return None
        elif len(models) == 1:
            return models[0]
    ...
    def upload_model_to_registry(is_default_version: bool, parent_model_uri: str = None) -> Model:
        ...
        vertex_model.uri = (
            f"https://{project_location}-aiplatform.googleapis.com/v1/" f"{uploaded_model.versioned_resource_name}"
        )
    ...
    def import_evaluation(
        ...
        schema = f"gs://google-cloud-aiplatform/schema/modelevaluation/" f"{problem_type}_metrics_1.0.0.yaml"
        evaluation = {
        ...
        client = ModelServiceClient(client_options={"api_endpoint": project_location + "-aiplatform.googleapis.com"})
    ...
    champion_model = lookup_model(project_id=project_id, project_location=project_location, model_name=model_name)
    ...
    if champion_model is None:
        ...
        # Compare models
        logging.info(f"Model default version {champion_model.version_id} " "is being challenged by new model.")
